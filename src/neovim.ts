...
export class NeovimManager {
	...
	public async sendCommand(command: string): Promise<string> {
		...
		try {
			...
			if (
				error instanceof NeovimCommandError ||
				error instanceof NeovimValidationError
			) {
				throw error;
			}
	...
	public async getNeovimStatus(): Promise<NeovimStatus | string> {
		try {
			...
			try {
				...
				const lspClients = await nvim.eval(
					'luaeval("vim.lsp.get_active_clients()")',
				);
				if (Array.isArray(lspClients) && lspClients.length > 0) {
					const clientNames = lspClients
						.map((client: any) => client.name || "unknown")
						.join(", ");
			...
			try {
				...
				const hasTreesitter = await nvim.eval(
	...
	public async saveBuffer(filename?: string): Promise<string> {
		try {
			...
			if (filename) {
				...
				if (!bufferName) {
					throw new NeovimValidationError(
						"Cannot save unnamed buffer without specifying filename",
					);
				}
	...
	public async searchInBuffer(
		...
		try {
			...
			const matches = await nvim.eval(
				`searchcount({"pattern": "${searchPattern.replace(/"/g, '\\"')}", "maxcount": 100})`,
			);
			const matchInfo = matches as {
				current: number;
				total: number;
				maxcount: number;
				incomplete: number;
	...
	public async searchAndReplace(
		pattern: string,
		replacement: string,
		options: { global?: boolean; ignoreCase?: boolean; confirm?: boolean } = {},
	): Promise<string> {
		if (!pattern || pattern.trim().length === 0) {
			throw new NeovimValidationError("Search pattern cannot be empty");
		}
	...
	public async grepInProject(
		...
		try {
			...
			const summary = results
				.slice(0, 10)
				.map((item) => `${item.filename}:${item.lnum}: ${item.text.trim()}`)
				.join("\n");
			...
			switch (action) {
				case "record":
					if (!register || register.length !== 1 || !/[a-z]/.test(register)) {
						throw new NeovimValidationError(
							"Register must be a single letter a-z for recording",
						);
					}
				...
				case "play":
					if (!register || register.length !== 1 || !/[a-z]/.test(register)) {
						throw new NeovimValidationError(
							"Register must be a single letter a-z for playing",
						);
					}
					const playCommand =
						count > 1 ? `${count}@${register}` : `@${register}`;
			...
			switch (action) {
				case "create":
					if (startLine === undefined || endLine === undefined) {
						throw new NeovimValidationError(
							"Start line and end line are required for creating folds",
						);
					}
	...
	public async navigateJumpList(direction: string): Promise<string> {
		try {
			...
			switch (direction) {
				...
				default:
					throw new NeovimValidationError(
						`Unknown jump direction: ${direction}`,
					);
			}
		} catch (error) {
			if (error instanceof NeovimValidationError) {
	...
	public async analyzeRelatedFiles(filename?: string): Promise<string> {
		try {
			...
			const buffer = filename
				? await this.findBufferByName(nvim, filename)
				: await nvim.buffer;
			const bufferName = await buffer.name;
	...
	public async findWorkspaceSymbols(
		query: string = "",
		limit: number = 20,
	): Promise<string> {
		try {
			...
			const hasLsp = (await nvim.eval(
				'exists(":LspWorkspaceSymbol")',
			)) as number;
			if (!hasLsp) {
	...
	public async getProjectStructure(): Promise<string> {
		try {
			...
			const output = (await nvim.eval(
				`systemlist('find . -type f -name "*.js" -o -name "*.ts" -o -name "*.py" -o -name "*.lua" -o -name "*.vim" -o -name "*.md" | head -100')`,
			)) as string[];
	...
	public async getGitStatus(): Promise<string> {
		try {
			...
			// Get git status
			const status = (await nvim.eval(
				'system("git status --porcelain 2>/dev/null")',
			)) as string;
	...
	public async getLspDiagnostics(): Promise<string> {
		try {
			...
			// Get all diagnostics
			const diagnostics = (await nvim.eval(
			...
			for (const diag of diagnostics) {
				...
				diagnosticsByBuffer[bufferName].push({
					...
					column: diag.col + 1,
					severity:
						["Error", "Warning", "Information", "Hint"][diag.severity - 1] ||
						"Unknown",
			...
			return JSON.stringify({
				error: "Could not get LSP diagnostics",
				totalCount: 0,
				diagnostics: {},
			});
		}
	...
	public async getWorkspaceContext(): Promise<string> {
		try {
			...
			const context = {
				...
				diagnosticSummary: {
					errors: (await nvim.eval(
						'luaeval("#vim.tbl_filter(function(d) return d.severity == 1 end, vim.diagnostic.get())")',
					)) as number,
					warnings: (await nvim.eval(
						'luaeval("#vim.tbl_filter(function(d) return d.severity == 2 end, vim.diagnostic.get())")',
					)) as number,
				},
